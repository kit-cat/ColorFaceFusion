...
namespace Microsoft.Kinect.Toolkit.FaceTracking
{
    ...
    [DebuggerDisplay("(W={Weight},l={Rect.Left},t={Rect.Top},r={Rect.Right},b={Rect.Bottom})")]
    [StructLayout(LayoutKind.Sequential)]
    public struct WeightedRect
    {
        public float Weight { get; set; }   // some weight 
        public Rect Rect { get; set; }     // rectangle coordinates as RECT structure
    };
    ...
    public struct Rect
    {
        ...
        {
            int maxLeft = Math.Max(this.Left, other.Left);
            int maxTop = Math.Max(this.Top, other.Top);

            int minRight = Math.Min(this.Right, other.Right);
            int minBottom = Math.Min(this.Bottom, other.Bottom);

            int width = Math.Max(0, minRight - maxLeft);
            int height = Math.Max(0, minBottom - maxTop);

            return new Rect(maxLeft, maxTop, maxLeft + width, maxTop + height);
        }
    }
